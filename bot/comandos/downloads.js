        ...
        switch(comandoSemPrefixo){      
            case "dl":
                try{
                    ...
                    if(resultadoInfoVideo.isLiveContent) return await socket.responderTexto(c, id_chat, comandos_info.downloads.dl.msgs.erro_live,mensagem)
                    else if (resultadoInfoVideo.lengthSeconds > 300) return await socket.responderTexto(c, id_chat, comandos_info.downloads.dl.msgs.limite, mensagem)
                    const mensagemEspera = criarTexto(comandos_info.downloads.dl.msgs.espera, resultadoInfoVideo.title, resultadoInfoVideo.durationFormatted)
